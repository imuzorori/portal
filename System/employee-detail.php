...
try {
	...
	$current_year = date('Y');
	$myage = $current_year - $byear;// corrections
	$myedu = $row['education'];
	$mytitle = $row['title'];
	$mymail = $row['email'];
	}
...
<head>
	...
	<title>TelmeIn Raco - <?php echo "$myfname"; ?> <?php echo "$mylname"; ?></title>
				...
				<div class="modal-body">
					...
					<div class="row gap-20">
						...
						<div class="col-sm-6 col-md-6">
							<a href="register.php?p=Employer" class="btn btn-facebook btn-block mb-5-xs">Sign-up Employer</a>
						</div>
						<div class="col-sm-6 col-md-6">
							<a href="register.php?p=Employee" class="btn btn-facebook btn-block mb-5-xs">Sign-up Employee</a>
						</div>
							...
							<div class="col-md-10 col-md-offset-1">
								...
								<div class="employee-detail-wrapper">
									...
									<div class="employee-detail-header text-center">
										...
										<p class="location"><i class="fa fa-map-marker"></i> <?php echo "$mycity"; ?>, <?php echo "$mycountry"; ?><span class="mh-5">|</span> <i class="fa fa-phone"></i> <?php echo "$myphone"; ?></p>
									...
									<div class="employee-detail-company-overview mt-40 clearfix">
										...
										<div class="row">
											...
											<div class="col-sm-12">
												...
												<p class="text-muted font-italic">Level - <?php echo $row['level']; ?> , <?php echo $row['timeframe']; ?><span class="font600 text-primary"> <?php echo $row['institution']; ?></span> <?php echo $row['city']; ?></p>
										...
										<h3>Professional Qualifications</h3>
                                                ...
                                                $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
                                                $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                                                $stmt = $conn->prepare("SELECT * FROM tbl_profess_qualification WHERE member_no = :empid ORDER BY id");
	                                            $stmt->bindParam(':empid', $empid);
                                                $stmt->execute();
                                                $result = $stmt->fetchAll();
	                                            $rec = count($result);
										...
										<ul class="list-icon">
                                                ...
                                                $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
                                                $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                                                $stmt = $conn->prepare("SELECT * FROM tbl_references WHERE member_no = :empid ORDER BY id");
	                                            $stmt->bindParam(':empid', $empid);
                                                $stmt->execute();
                                                $result = $stmt->fetchAll();
	                                            $rec = count($result);
