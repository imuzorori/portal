...
$mystreet = $row['street'];
$mypostal_address = $row['postal_address'];
...
$pdf->Cell(0,9,''.$mygender.', '.$myemail.', '.$mycity.','.$mycountry.','.$mystreet.','.$mypostal_address.'',1,1,'C',true);
...
$stmt = $conn->prepare("SELECT * FROM tbl_profess_qualification WHERE member_no = '$myid'");
$stmt->execute();
$result = $stmt->fetchAll();
...
$stmt = $conn->prepare("SELECT * FROM tbl_references WHERE member_no = '$myid'");
$stmt->execute();
$result = $stmt->fetchAll();
